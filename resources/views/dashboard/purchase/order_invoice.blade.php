                ...
                <div class="panel panel-white">
                    <div class="panel-body">
                        ...
                        <form action="{{ route('purchase.order.invoice.store') }}" method="post">
                            @csrf
                            <input type="hidden" name="purchaseId" value="{{ $dataInfo->id }}">
                            <div class="table-responsive">
                                <table class="table table-striped" id="purchaseTable">
                                            ...
                                            <th width="5%" colspan="8" scope="col">
                                                ...
                                                <select name="supplier" class="form-control medicine-select" id="supplierList" required>
                                                    ...
                                                    @foreach ($suppliers as $sup)
                                                        <option {{ ($dataInfo->supplierId == $sup->id)?'selected':'' }} value="{{ $sup->id }}">{{ $sup->name }} || {{ $sup->phone }}</option>
                                                    @endforeach
                                                </select>
                                            </th>
                                        </tr>
                                    ...
                                    <tbody id="medicineTableBody">
                                        @foreach ($dataInfo->purchase_details as $key => $item)
                                            @php
                                                $sl = $key + 1;
                                            @endphp
                                            <tr id="row_{{$sl}}" data-row-number="{{ $sl }}">
                                                <td>{{$sl}}</td>
                                                <td>
                                                    <select name="medicine[]" class="form-control medicine-select" onchange="getMedicineDetail(this);" id="medList_{{$sl}}" required>
                                                        <option value="" disabled selected>{{ __('Select Medicine') }}</option>
                                                        @foreach ($medicines as $med)
                                                            <option {{ ($med->id == $item->medicineId) ? 'selected' : '' }} value="{{ $med->id }}">{{ $med->name }} ({{ $med->generic_name }},{{ $med->strength }} ) {{ $med->supplier->name }}</option>
                                                        @endforeach
                                                    </select>
                                                </td>
                                                <td>
                                                    <input type="date" name="expire_date[]" class="form-control expire_date" min="1" id="expire_date_{{$sl}}" required />
                                                </td>
                                                <td>
                                                    <input type="number" name="qty[]" value="{{ $item->qty }}" class="form-control qty" min="1" id="qty_{{$sl}}" required />
                                                </td>
                                                <td>
                                                    <input type="text" name="price[]" value="{{ $item->price }}" class="form-control price" readonly id="price_{{$sl}}" required />
                                                </td>
                                                <td>
                                                    <input type="text" name="total[]" value="{{ $item->total }}" class="form-control total" id="total_{{$sl}}" readonly/>
                                                </td>
                                                <td>
                                ...
                                <div class="col-md-11">
                                    <div class="form-row">
                                        ...
                                        <div class="form-group col-md-2">
                                            <input type="number" step="0.00" class="form-control" readonly id="total_dicount" value="0.00" placeholder="{{ __('Total Discount') }}">
                                            <small class="text-muted">{{ __('Total Discount') }}</small>
                                        </div>
                                        <div class="form-group col-md-2">
                                            <input type="number" class="form-control" name="grandTotal" id="grandTotal" value="{{ $dataInfo->grand_total }}" step="0.00" readonly>
                                            <small class="text-muted">{{ __('Total Invoice Amount') }}</small>
                                        </div>
                                        <div class="form-group col-md-2">
                                            <input type="number" class="form-control" name="paidAmount" id="paidAmount" value="{{ $dataInfo->grand_total }}" step="0.00">
                                            <small class="text-muted">{{ __('Payable Amount') }}</small>
                                        </div>
                                        <div class="form-group col-md-2">
                                            <input type="number" class="form-control" name="dueAmount" id="dueAmount" value="0.00" step="0.00" readonly>
